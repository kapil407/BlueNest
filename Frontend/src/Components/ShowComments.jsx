...
const GetComment=({id})=>{
    ...
    return(
            <div className={` inline h-35 flex ml-2 overflow-auto  mt-1 rounded mr-2 p-2 ${theme=="light"?"":"text-gray-400 "}`}>
               {
                CommentContent && CommentContent?.map((ele)=>(
                    <div key={ele._id} className="break-words ">
                       <div className=" rounded whitespace-pre-wrap inline-block max-w-[75%] my-4 flex flex-col">
                        <div className="flex">
                            <img src={user?.profilePic?.url} alt="" className="object-cover h-10 w-10 rounded-full" />
                        ...
                        <div className="flex justify-between ">
                            <MdOutlineDeleteOutline className="ml-4 cursor-pointer" size={20}/>
